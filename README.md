...
Settings live in `~/.pupa-backend/config.yml` — written by the wizard, safe to
edit by hand. API keys for your model provider stay in your shell environment,
...
- **[Cloudflare tunnel](https://www.cloudflare.com/products/tunnel/)** — gives
  ...
  domain — set it up once in `pupa-backend setup`.
...
the same pairing, and — the important part — **your app's own tools**. The app
...
lets you choose per connection. Anything else — MCP servers, tracing, screen
