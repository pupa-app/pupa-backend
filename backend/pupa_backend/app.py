...
# `cloudflared` reads this on every invocation unless `--config` says otherwise.
# The named-tunnel path wants it; the quick tunnel must not have it (see
# `_QUICK_TUNNEL_CONFIG`).
_CLOUDFLARED_USER_CONFIG = Path.home() / ".cloudflared" / "config.yml"

# Isolated config for the quick tunnel. Comment-only on purpose: `--url` supplies
# the whole ingress, but cloudflared rejects a zero-byte file outright
# ("Configuration file … was empty").
_QUICK_TUNNEL_CONFIG = Path.home() / ".pupa-backend" / "cloudflared-quick.yml"
...
def _start_cloudflared_tunnel() -> "subprocess.Popen[str] | None":
    ...
    Runs against an isolated `--config`. Without it cloudflared picks up
    `~/.cloudflared/config.yml`, and on a machine that has been through the
    named-tunnel setup that file's mandatory catch-all is `http_status:404`:
    the quick tunnel's random `*.trycloudflare.com` Host matches no `hostname:`
    rule, so cloudflared answers every request itself with a bodiless 404 and
    nothing reaches uvicorn. Nothing shows up in the backend log either — and
    the client reads the 404 on `POST /auth/pair` as a spent pairing code.
    """
    if not shutil.which("cloudflared"):
        logger.warning("connectivity=cloudflared but cloudflared not found — install: brew install cloudflared")
        return None

    if _CLOUDFLARED_USER_CONFIG.exists():
        logger.warning(
            "quick tunnel: ignoring %s — its ingress rules only match the named "
            "tunnel's hostname and would 404 every request to the "
            "trycloudflare.com URL. To serve that hostname instead, set "
            "cloudflared.hostname + cloudflared.tunnel in ~/.pupa-backend/config.yml.",
            _CLOUDFLARED_USER_CONFIG,
        )

    _QUICK_TUNNEL_CONFIG.parent.mkdir(parents=True, exist_ok=True)
    _QUICK_TUNNEL_CONFIG.write_text(
        "# Generated by pupa-backend. Deliberately empty of ingress rules: the\n"
        "# quick tunnel's whole ingress comes from `--url`. Exists only to keep\n"
        "# cloudflared off ~/.cloudflared/config.yml.\n"
    )

    proc = subprocess.Popen(
        # `--config` is a global flag — it must precede the `tunnel` subcommand.
        ["cloudflared", "--config", str(_QUICK_TUNNEL_CONFIG),
         "tunnel", "--url", "http://localhost:8004"],
